    ...
    }
    {
        let db = databases.get(DB_NAME)?;
        {
            let session = Session::new(db, SessionType::Schema)?;
            {
                ...
                // end::get_type[]
                // tag::add_type[]
                let admin = tx.concept().put_entity_type("admin".to_owned()).resolve()?;
                // end::add_type[]
                // tag::set_supertype[]
                let _ = admin.set_supertype(&tx, user);
                // end::set_supertype[]
                // tag::get_instances[]
                ...
                // tag::create[]
                let new_user = tx.concept().get_entity_type("user".to_owned()).resolve()?.unwrap().create(&tx).resolve();
                // end::create[]
                // tag::delete_user[]
